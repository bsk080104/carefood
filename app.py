...
if st.button("맞춤 식품 추천 결과 보기"):
    ...
    if "딱딱한 과일이나 고기도 잘 씹을 수 있음" in chew:
        ...
        if disease_count >= 2:
            ...
            food_recommend = "기타 질환 1개용 일반식"

    elif "부드러운 고기나 나물류 선호" in chew:
        texture_grade = "연화식 위주 식단 필요"
        if disease_count >= 2:
            food_recommend = "복합질환용 부드러운 음식"
        elif bp:
            food_recommend = "고혈압용 부드러운 음식"
        elif "일반 건강군" in selected_diseases:
            food_recommend = "건강군용 연화식"
        else:
            food_recommend = "기타 질환 1개용 연화식"

    else:
        texture_grade = "액체류 식단 필요"
        if disease_count >= 2:
            food_recommend = "복합질환용 죽"
        elif sugar_dis:
            food_recommend = "당뇨용 죽"
        elif "일반 건강군" in selected_diseases:
            food_recommend = "건강군용 죽"
        else:
            food_recommend = "기타 질환 1개용 죽"
